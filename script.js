...
/* ---------- Prediction for a city (client & server modes) ---------- */
...
async function fetchLiveForCity(city) {
  ...
  try {
    if (WEATHERAPI_KEY) {
      ...
      if (r.ok) {
        weatherAPI_raw = await r.json();
        debug('WeatherAPI raw', weatherAPI_raw);
        const w = weatherAPI_raw;
  ...
  try {
    if (OPENWEATHER_KEY) {
      ...
      if (r.ok) {
        openWeather_raw = await r.json();
        debug('OpenWeather raw', openWeather_raw);
        const ow = openWeather_raw;
